...
export class BrowserTitlebarPart extends Part implements ITitlebarPart {
	...
	private mcuSelector: HTMLButtonElement | undefined;
	private stLinkIndicator: HTMLElement | undefined;
	...
	private createCubeForgeControls(): void {
		if (this.isAuxiliary || this.isCommandCenterVisible) {
			return;
		}

		const controls = append(this.centerContent, $('div.cubeforge-title-controls'));
		this.mcuSelector = append(controls, $('button.cubeforge-mcu-selector', { type: 'button' })) as HTMLButtonElement;
		this.stLinkIndicator = append(controls, $('span.cubeforge-stlink'));

		this.mcuSelector.textContent = this.configurationService.getValue<string>('tova.mcu.default') || 'STM32F446RE';
		this.mcuSelector.title = localize('cubeforge.selectMcu', 'Select MCU');
		this.stLinkIndicator.textContent = localize('cubeforge.stlink.unknown', '○ ST-LINK');

		this._register(addDisposableListener(this.mcuSelector, EventType.CLICK, async () => {
			const selected = await this.quickInputService.pick([
				{ label: 'STM32F446RE' },
				{ label: 'STM32F103C8' },
				{ label: 'STM32H743ZI' }
			], {
				placeHolder: localize('cubeforge.selectMcu.placeholder', 'Select MCU')
			});
			if (selected && this.mcuSelector) {
				this.mcuSelector.textContent = selected.label;
			}
		}));
	}
...
export class MainBrowserTitlebarPart extends BrowserTitlebarPart {
	...
	constructor(
		...
		super(Parts.TITLEBAR_PART, mainWindow, editorGroupService.mainPart, contextMenuService, configurationService, environmentService, instantiationService, themeService, storageService, layoutService, contextKeyService, hostService, editorService, menuService, keybindingService, quickInputService);
	}
}
...
export class AuxiliaryBrowserTitlebarPart extends BrowserTitlebarPart implements IAuxiliaryTitlebarPart {
	...
	constructor(
		...
		super(`workbench.parts.auxiliaryTitle.${id}`, getWindow(container), editorGroupsContainer, contextMenuService, configurationService, environmentService, instantiationService, themeService, storageService, layoutService, contextKeyService, hostService, editorService, menuService, keybindingService, quickInputService);
	}
